...
addEventHandler('getFilenamesFromResource-c', resourceRoot, function(resourceName, filesTable, resourceRunning)
    ...
    if (not filesTable or #filesTable < 1) then
        outputConsole(inspect(filesTable));
        executeBrowserJavascript(b, [[
            iziToast.error({
                title: 'Błąd',
...
addEventHandler('createNotification', resourceRoot, function(type, title, text)
    ...
    local b = guiGetBrowser(guiBrowser);
    executeBrowserJavascript(b, "iziToast."..type.."({id: 'success', zindex: 9999, timeout: 3000, title: "..inspect(title)..", overlay: true, message: "..inspect(text)..", position: 'center'});")
